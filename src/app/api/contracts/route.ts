...
const contractsHandler = require('../../../../lambda/functions/contracts-service/index.js').handler;
...
async function executeLambdaOrFallback(action: string, payload: any) {
  const functionName = 'contracts-service';
  ...
  const res = await contractsHandler({ action, payload });
...
export async function PUT(req: NextRequest) {
  const payload = await req.json();
  const result = await executeLambdaOrFallback('SIGN', payload);
  return NextResponse.json(result);
}
