...
export default function PublicEnquiryPage({ params }: { params?: { id?: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const rawBudget = (showCalculator ? String(calculatedTotal) : formData.budget).replace(/[^0-9]/g, '');
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-[#00d4ff] selection:text-black antialiased font-sans">
      {/* Top Navbar */}
      <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#00d4ff] via-[#3b82f6] to-[#8b5cf6] flex items-center justify-center shadow-lg shadow-sky-500/20">
            <Camera className="w-5 h-5 text-white" />
          </div>
          <div>
            <div className="flex items-center space-x-2">
              <span className="font-black text-lg tracking-tight text-white">PIXEVA</span>
        ...
        <div className="flex items-center space-x-3">
          <a
            ...
            className="flex items-center space-x-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold hover:bg-emerald-500/20 transition-colors"
          >
      ...
      <div className="relative h-72 md:h-96 w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end max-w-4xl mx-auto px-6 pb-10 text-center">
          <div className="inline-flex items-center justify-center space-x-2 px-3.5 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs font-semibold text-sky-300 mx-auto mb-3">
            <Sparkles className="w-3.5 h-3.5 text-[#00d4ff]" />
            <span>Official Booking & Consultation Form</span>
          </div>
          <h1 className="text-3xl md:text-5xl font-black text-white tracking-tight drop-shadow-lg">
            {headline}
          </h1>
          <p className="text-slate-300 text-sm md:text-base mt-2 max-w-xl mx-auto drop-shadow-md">
            {subtitle}
          </p>
        </div>
      </div>
      ...
      {/* Main Container */}
      <main className="max-w-3xl mx-auto px-4 sm:px-6 -mt-6 pb-20 relative z-10">
        {submitted ? (
          /* Confirmation State */
          <div className="p-8 md:p-12 rounded-3xl bg-slate-900/90 border border-white/15 backdrop-blur-2xl shadow-2xl text-center space-y-6 animate-fadeIn">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-400 text-sm max-w-md mx-auto">
                Thank you, <span className="font-bold text-white">{formData.name}</span>! Our creative director is reviewing your event details for{' '}
                <span className="text-[#00d4ff] font-semibold">{formData.event_date}</span>.
              </p>
            </div>

            <div className="p-5 rounded-2xl bg-slate-950 border border-white/10 text-left space-y-3 max-w-lg mx-auto text-xs">
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center justify-center space-x-2 shadow-lg shadow-emerald-600/25 transition-colors"
              >
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-white/10 hover:bg-white/15 text-white font-semibold text-xs transition-colors"
              >
          ...
          <form
            ...
            className="p-6 md:p-10 rounded-3xl bg-slate-900/90 border border-white/15 backdrop-blur-2xl shadow-2xl space-y-6"
          >
